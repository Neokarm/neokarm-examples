...
This example shows you how to use Terraform and Packer to create an Ubuntu AMI with Docker installed. The AMI is stored within zCompute, where users can easily access and deploy it.
...
* Once inside the temporary instance, Packer runs the provisioner commands to install Docker, clones the Docker-provisioned instance into an output AMI, and uploads the output AMI into zCompute.
...
* First, do some setup tasks as describe below within zCompute.
...
2. **Upload the image (or images)** you will use for both the Bastion and Packer instances into zCompute:

    This example uses this same Ubuntu Xenial cloud image for both Bastion and Packer instances:
    
    https://cloud-images.ubuntu.com/xenial/current/xenial-server-cloudimg-amd64-disk1.img
    
    To upload the image into zCompute:
...
3. Get the **AMI ID** for the image you just uploaded into zCompute:
...
The following variables: **zCompute_ip**, **access_key**, **secret_key** are described [here](../ec2-instance/README.md).
...
5. Run `packer build packer_generated.json`. This command creates the Docker-provisioned output AMI and uploads it into zCompute.

    Notes:
    
    * This example automatically creates the `packer_generated.json` file, based on the template contained in `packer_gen.template`.
    
    * For the `packer build` command to succeed, your zCompute region must have a valid certificate.
    
    * Make sure you are using the right packer rpm.

6. Verify that the output AMI is available in zCompute:
